...
// Immediately assign approximate bounding boxes by distributing sections
// proportionally across pages — used as the instant first pass.
function assignProportional(sections, numPages) {
  const perPage = Math.ceil(sections.length / numPages)
  return sections.map((section, idx) => {
    if (section.geometry) return section
    const page = Math.min(numPages, Math.floor(idx / perPage) + 1)
    const pageIdx = idx % perPage
    const numOnPage = Math.min(perPage, sections.length - Math.floor(idx / perPage) * perPage)
    const segH = 0.85 / numOnPage
    return {
      ...section,
      geometry: {
        page,
        boundingBox: { Left: 0.01, Top: 0.07 + pageIdx * segH, Width: 0.98, Height: segH * 0.92 },
      },
    }
  })
}

// Compute a bounding box from PDF.js text items that overlap the section text.
function computeBboxFromItems(sectionText, { pageText, items, viewport }) {
  const probe = normStr(sectionText)
  const short = probe.slice(0, 30)
  const offset = pageText.indexOf(short)
  if (offset === -1) return null

  const sectionEnd = offset + probe.length
  ...
  for (const item of items) {
    const s = normStr(item.str || '')
    if (!s) continue
    const idx = pageText.indexOf(s, Math.max(0, pos - 2))
    if (idx === -1) continue
    const itemEnd = idx + s.length
    pos = itemEnd

    if (itemEnd <= offset) continue
    if (idx >= sectionEnd) break

    const [, , , , tx, ty] = item.transform
    const iH = item.height || 10
    const iW = item.width || 0
    minX = Math.min(minX, tx / W)
    minY = Math.min(minY, (H - ty - iH) / H)
    maxX = Math.max(maxX, (tx + iW) / W)
    maxY = Math.max(maxY, (H - ty) / H)
    found = true
  }

  if (!found) return null
  return {
    Left:   Math.max(0,    minX - 0.01),
    Top:    Math.max(0,    minY - 0.005),
    Width:  Math.min(1,    maxX - minX + 0.02),
    Height: Math.min(0.95, maxY - minY + 0.01),
  }
}

// Async: load the PDF via PDF.js, extract per-page text + item positions,
// match each section to the correct page and compute a tight bounding box.
async function assignGeometryFromPdf(pdfUrl, sections) {
  ...
  for (let p = 1; p <= pdf.numPages; p++) {
    ...
    const pageText = normStr(items.map(i => i.str).join(' '))
    pageData.push({ pageNum: p, pageText, items, viewport })
  }

  // Assign each section to a page
  const assignments = sections.map((section, idx) => {
    if (section.geometry) return { section, pageNum: section.geometry.page, pd: null }
    const probe = normStr(section.originalText || '').slice(0, 30)
    for (const pd of pageData) {
      if (probe.length > 8 && pd.pageText.includes(probe)) {
        return { section, pageNum: pd.pageNum, pd }
      }
    }
    // Proportional fallback page
    const pageNum = Math.min(pdf.numPages, Math.floor(idx * pdf.numPages / sections.length) + 1)
    return { section, pageNum, pd: pageData[pageNum - 1] || null }
  })

  // Stack sections within each page for the fallback bounding box
  const pageGroups = {}
  for (const a of assignments) {
    ;(pageGroups[a.pageNum] ??= []).push(a)
  }

  return assignments.map((a, idx) => {
    if (a.section.geometry) return a.section

    let boundingBox = a.pd ? computeBboxFromItems(a.section.originalText, a.pd) : null

    if (!boundingBox) {
      const group   = pageGroups[a.pageNum]
      const pos     = group.findIndex(g => g.section.sectionId === a.section.sectionId)
      const segH    = 0.85 / group.length
      boundingBox   = { Left: 0.01, Top: 0.07 + pos * segH, Width: 0.98, Height: segH * 0.92 }
    }

    return { ...a.section, geometry: { page: a.pageNum, boundingBox } }
  })
}
...
export default function DirectFillScreen() {
  ...
  // Assign geometry to sections once the PDF has loaded and we know numPages.
  // First pass is instant (proportional); second pass refines via PDF.js text extraction.
  useEffect(() => {
    if (!numPages || !session.sections?.length || !session.pdfUrl) return
    setEnrichedSections(assignProportional(session.sections, numPages))

    let cancelled = false
    assignGeometryFromPdf(session.pdfUrl, session.sections)
      .then(result => { if (!cancelled) setEnrichedSections(result) })
      .catch(err => console.warn('PDF geometry refinement failed:', err))
    return () => { cancelled = true }
  }, [numPages, session.pdfUrl, session.sections])
  ...
  // Use geometry-enriched sections for overlay rendering; fall back to raw sections
  // before the PDF has loaded (enrichedSections is null until numPages is known).
  const displaySections = enrichedSections || sections

  // Sections visible on current page
  const pageSections = displaySections.filter(s => s.geometry?.page === pageNumber)
